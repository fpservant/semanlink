...
private JFileBiModel getJFileBiModel4Docs(String docUri) throws JenaException, IOException, URISyntaxException {
	JFileModel smallJFileModel = getJFileModel4Docs(docUri);
	return new JFileBiModel(this.docsModel, smallJFileModel);
}
...
/**
 * Ajouter un docListener.
 * Mon but initial était d'attacher une date de création à un doc (ou + généralement, attacher
 * automatiquement des metadata)
 * Pb : un doc est créé par le fait qu'il y a création d'un statement parlant de lui.
 * mais tout subject est-il un doc ? On a d'abord supposé ici que oui, mais
 * posait un pb avec, par ex, les local copy (qu'on prenait pour des docs)
 * -> regarder la ppté : si ppté est hasKw, subject est un doc : pas bon (en général, on ajoute
 * d'abord d'autres ppts, et du coup)
 * -> hack : si source, je ne considère pas new doc -- EN ATTENDANT - TODO -- de
 * changer ça (: créer une ppté diecte hasLocalCopy ? Abandonner le listener pour les new docs ?
 */
...
class DocListener extends StatementListener { // NO MORE USED
	JModel jm;
	DocListener(JModel jm) { this.jm = jm; }
	public void addedStatement(Statement sta) {
		try {
			Resource subject = sta.getSubject();

			/* Dealing with new docs : this now is done by the addDocProperty and setDocProperty methods
			// is this statement about a new document?
			// If it is, we want to do some things about it,
			// such as adding a creation date.
			// So, is subject already present in the model? Are there already statements about it?
			// Beware: now that we are in addedStatement, sta must already be in the model
			// -> we look if there are others statements about subject
			// (Why don't I simply test whether subject has a creation date property?
			// because at one time, I didn't systematicaly add a creation date prop to documents,
			// so I have documents that do not have creation date prop. I don't want to add them as creation date
			// the date of the first modif
			StmtIterator ite = subject.listProperties ();
			boolean newDoc = true;
			// we don't want a local copy of a doc to be considered a doc (with properties such as
			// creation data etc.), hence the test on statement's predicate (a local copy
			// has its source prop pointing to the actual document) (If the subject is a real doc,
			// it will have others props set: we won't miss to send the addedDoc event)
			if (!(sta.getPredicate().getURI().equals(SLVocab.SOURCE_PROPERTY))) {
				for (;ite.hasNext();) {
					if (sta.equals(ite.next())) continue;
					newDoc = false;
					break;
				}
				ite.close();
			...
			if (newDoc) {
				addedDoc(new JDocument(this.jm, subject));
			}
			*/
			
			// GESTION DE L'ECRITURE DU FICHIER MODIFIE 
			String subjectUri = subject.getURI();
			JFileModel jfm = getJFileModel4Docs(subjectUri);
			Model mo = jfm.getModel();
			JenaUtils.add(mo, subjectUri, sta.getPredicate().getURI(), sta.getObject().inModel( mo));
			//System.out.println("JModel added : " + subjectUri + " " + sta.getPredicate().getURI() + " " + sta.getObject().inModel( mo));
			jfm.save();
		} catch (Exception e) {
			throw new SLRuntimeException(e);
		}
	...
	public void removedStatement(Statement sta)  {
		try {
			// GESTION DE L'ECRITURE DU FICHIER MODIFIE 
			Resource subject = sta.getSubject();
			String subjectUri = subject.getURI();
			JFileModel jfm = getJFileModel4Docs(subjectUri);
			Model mo = jfm.getModel();
			System.out.println("JModel removedStatement : " + subjectUri + " " + sta.getPredicate().getURI() + " " + sta.getObject().inModel( mo));
			// CECI N'A PAS L'AIR DE MARCHER BIEN ???
			JenaUtils.remove(mo, subjectUri, sta.getPredicate().getURI(), sta.getObject().inModel( mo));
			//jfm.getFile().getPath();
			//
			jfm.save();
		} catch (Exception e) {
	...
	/**
	 * get called when a new doc is added to the model.
	 * -> ajout de la date de création
	 * -> metadata extraction
	 */
	/*
	public void addedDoc(SLDocument doc) throws Exception {
    YearMonthDay today = new YearMonthDay();
		this.jm.addDocProperty(doc, SL_CREATION_DATE_PROPERTY, today.getYearMonthDay("-"), null);
		...
		if (file != null) {
			if (file.exists()) {
				...
				String modifVal = modifDay.getYearMonthDay("-");
				// if (modifVal.compareTo("2005-11-01") > 0) {
				    this.jm.addDocProperty(doc, DATE_PARUTION_PROPERTY, modifVal, null);
				// }
			}
		}
		try {
			MetadataExtractorManager.getInstance().doIt(doc, this.jm);
		} catch (Throwable e) { // on ne veut pas planter
			System.err.println("Error extracting metadata for " + doc.getURI() + " : " + e);
		}
	}
	*/
} // class DocListener
