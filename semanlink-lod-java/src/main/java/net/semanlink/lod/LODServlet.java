...
import net.semanlink.util.httpClientFacade.HttpClientFactory;
import net.semanlink.util.httpClientFacade.HttpClientInterface;
import net.semanlink.util.jena.RDFServletWriterUtil;
import net.semanlink.util.servlet.Jsp_Page;

abstract public class LODServlet extends SparqlServlet {
	...
	if (servletPath.endsWith("/get")) { // /get?uri=...
		...
		if (!this.dataset.owns(uri)) {
			...
			*/
			...
			HttpClientInterface httpClient = HttpClientFactory.createInstance();
