...
RDFFormula.prototype.literal = function(val, lang, dt) {
	// fps
	// when val is an XML Literal (containing a chunck of XML)
	// exception here with ie (6 at least) :
	// "cet objet ne gere pas cette propriete ou methode"
	// (this object doens't supprt this property or method)
	// You cannot test if (val.toString())
	try {
	    return new RDFLiteral(val.toString(), lang, dt)
	} catch (e) {
	    return new RDFLiteral("terms.js unable to transform that literal to string: " + e.toString(), lang, dt);
	}
}
