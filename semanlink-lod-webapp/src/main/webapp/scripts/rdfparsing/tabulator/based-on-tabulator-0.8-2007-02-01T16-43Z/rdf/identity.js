...
// modifications to the code of tabulator-0.8-2007-02-01T16-43Z
// by francois-paul.servant@renault.com
// to get it working with internet explorer (6 and 7)
// and Safari (3)
// Changes tagged with "// fps"
...
// fps In some cases, you know that you cannot set privileges, but don't need them either.
// hence a non required param dontNeedPrivileges added to load function
// @param noAlertIfError (optional) if true, no alert raised in case of error (default to false)
// RDFIndexedFormula.prototype.load = function(url) {
RDFIndexedFormula.prototype.load = function(url, dontNeedPrivileges, noAlertIfError) {
    // get the XML
	...
	// fps
	// code of tabulator-0.8-2007-02-01T16-43Z does not work with
	// ie7 win xp sp2, when the server fails to set the Content-Type to text/xml
	// (tested with
	// Internet Explorer version 7.0.5730.11CO Build 75730
	// Windows XP Professional 5.1.2600 Service Pack 2 Build 2600)
	
    var nodeTree = xhr.responseXML;
    
    // fps "ie7 win xp sp2 content-type != text/xml": 
    // nodeTree != null but nodeTree.childNodes.length == 0
    // There doesn't seem to be any parse error:
    // alert("parseError.errorCode: " + nodeTree.parseError.errorCode); // "0"
    
	// Therefore, change the test 
    // if (nodeTree === null && xhr.responseText != null) {
	// to:
    // if ( ((nodeTree == null)||(nodeTree.childNodes.length == 0)) && xhr.responseText !== null) {
    // I have seen somewhere that we could use
    // if ( ((nodeTree == null)||(nodeTree.documentElement == null)) && xhr.responseText !== null) {
    //
	// But now that the test returns true, we get a Runtime Error:
	// Error: 'DomParser' is undefined
	// Hence, changed the original code:
	/*
     if (nodeTree === null && xhr.responseText !== null) {
	// Only when the server fails to set Content-Type: text/xml AND xmlhttprequest doesn't have the overrideMimeType method
	nodeTree = (new DOMParser()).parseFromString(xhr.responseText, 'text/xml');
    } */
    // to:
	if ( ((nodeTree == null)||(nodeTree.childNodes.length == 0)) && xhr.responseText !== null) {
		// Probably, the server failed to set Content-Type: text/xml AND xmlhttprequest doesn't have the overrideMimeType method
		if (window.DOMParser) {
			nodeTree = (new DOMParser()).parseFromString(xhr.responseText, 'text/xml');
		} else if (window.ActiveXObject) {
			nodeTree = new ActiveXObject('Microsoft.XMLDOM');
			// could use this too:
			// nodeTree = new ActiveXObject('MSXML2.DOMDocument.3.0');
			nodeTree.async = false;
			// We need to turn off validation
			// or we get a parseError: "the element 'rdf:RDF' is used but not declared in the DTD/Schema."
			// (should we first try with validation, and only then without it, in case of the afforementionned error?)
			// Turning off validation:
			nodeTree.validateOnParse = false;
			try {
				nodeTree.loadXML(xhr.responseText);
				if (nodeTree.parseError) {
					if (nodeTree.parseError.errorCode != 0) {
						if(!noAlertIfError) alert("parseError.reason: " + nodeTree.parseError.reason + " srcText: " + nodeTree.parseError.srcText);
						nodeTree = null;
					}
				}
			} catch(er) {
				if(!noAlertIfError) alert("Exception when trying to loadXML " + url + " : " + er); // fps c'ets ce qui m'arrive quand tryingy to load les infos sur xml mr
				nodeTree = null;
			}
			 
		} else {
			if(!noAlertIfError) alert("Sorry, I tried, but I have not been able to figure out how to run a DOM Parser");
		}
    }

    // Get RDF statements fromm XML

	if (nodeTree) { // fps: nodeTest can indeed be null
